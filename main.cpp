...
uint32_t check_filter(unsigned char* data, int ret) {
	...
	char* web_start = strstr(payload, "Host: ");
	...
	char web[256] = {0, };
	int index = 0;
	for(index ; index < payload_len ; index++) {
		web[index] = web_start[index + 6];
		if(web_start[index + 6] == '\r')
			break;
	}	
	web[index] = '\n';

	if(memcmp(web, target_web, index) == 0 && strlen(web) == strlen(target_web)) {
		printf("blocked %s", web);
		return NF_DROP;
	}
	
	printf("allowed %s", web);
...
void usage() {
        printf("netfilter-test <host>\n");
        printf("sample : netfilter-test gilgil.net\n");
}
...
int main(int argc, char **argv)
{
	...
	memcpy(target_web, argv[1], strlen(argv[1]));
	target_web[strlen(argv[1])] = '\n';
